...
static esp_err_t capture_handler(httpd_req_t *req)
{
	...
	if (!fb)
	{
		if(wtd) Serial.println("Camera capture failed");
...
static esp_err_t streamHandler(httpd_req_t *req)
{
	...
	{
		fb = esp_camera_fb_get();
		if(wtd) Serial.print("Frame size ");
		if(wtd) Serial.println(fb->len);
		if (!fb)
		{
			if(wtd) Serial.println("Camera capture failed");
			continue;
...
void startCameraServer()
{
	...
	if(wtd) Serial.printf("Starting web server on port: '%d'\n", config.server_port);
	...
	config.ctrl_port += 1;
	if(wtd) Serial.printf("Starting stream server on port: '%d'\n", config.server_port);
